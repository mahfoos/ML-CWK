...
exchangeUSD <- read_excel("D:/Accadamic Materials/IIT/02nd Year/Second Semester/Machine Learnig/CourseWork/ExchangeUSD.xlsx") %>%
  janitor::clean_names() %>%
...
set.seed(123)
usd_train <- normalized_usd[3:400,]
usd_test <- normalized_usd[401:501,]
...
usd_min_test <- min(usd_exchange_full[401:501,3])
usd_max_test <- max(usd_exchange_full[401:501,3])
...
model_two_hidden_layers = function(hidden,sec_hidden) {
  ...
  truthcol = usd_exchange_full[401:501,3]$usd_eur
...
set_a_models_two_layers = results_two_hidden_layers %>%
  ...
  arrange(rmse)
kable(set_a_models_two_layers[3:10,])
